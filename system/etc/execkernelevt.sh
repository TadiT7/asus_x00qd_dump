...
if [ "$EXEC_REASON" == "dumpmem" ]; then
	...
	dumpsys meminfo -a > $DUMPSYS_PATH/${current_time}_dumpsys_meminfo.txt
        ...
        echo "***************Dump ion start ****************************" >>  $DUMPSYS_PATH/${current_time}_dumpsys_meminfo.txt
        cat /d/ion/heaps/system >> $DUMPSYS_PATH/${current_time}_dumpsys_meminfo.txt
        echo "***************Dump ion end*******************************" >>  $DUMPSYS_PATH/${current_time}_dumpsys_meminfo.txt
